...
session = requests_cache.CachedSession('rfi_monitor')
...
async def get_stations() -> list[RFIStation]:
  try:
    f = session.get(STATIONS_URL, headers=DEFAULT_HEADERS)
...
async def get_monitor(place_id: str, direction: str) -> list[RFIMonitorEntry]:
  ...
  return [RFIMonitorEntry(
    ...
    info=tr.select_one("#RDettagli .testoinfoaggiuntive").get_text().strip()
  ) for tr in monitor.select("tr[name='treno']")]
